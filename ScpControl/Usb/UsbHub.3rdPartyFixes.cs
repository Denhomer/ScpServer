...
namespace ScpControl.Usb
{
    public partial class UsbHub
    {
        ...
        /// <param name="path">The device path.</param>
        /// <returns></returns>
        private static bool Apply3RdPartyWorkaroundsForDs4(ref UsbDevice current, byte instance = 0x00, string path = default(string))
        {
            if (!(current is UsbDs4))
                return true;

            var padId = current.PadId;

            #region UltraStik workaround

            // if UltraStik is detected...
            if (current.VendorId == 0xD209 && (current.ProductId == 0x0501 || current.ProductId == 0x0502 || current.ProductId == 0x0503 || current.ProductId == 0x0504))
            {
                Log.InfoFormat(
                    "UltraStik detected [VID: {0:X4}] [PID: {1:X4}], workaround applied",
                    current.VendorId, current.ProductId);
                // ...close device...
                current.Close();
                // ...and create customized object
                current = new ScpControl.Usb.Gamepads.UsbUltraStik360(current.VendorId, current.ProductId)
                {
                    PadId = padId
                };
